"""
Nucleotide FASTA export module.

Exports nucleotide sequences from Parquet/DuckDB with >{gid} headers
for use with skani and other genomic tools.
"""
...
def export_nt_fasta(
    ...
    Args:
        output_fna: Output FASTA file path
    ...
    try:
        # Query nucleotide sequences
        query = "SELECT gid, sequence FROM sequences WHERE seq_type = 'NT'"
        params = []
        ...
        print(f"✓ Exported {count} nucleotide sequences to {output_fna}")
        return count
...
def export_nt_by_genomes(
    output_dir: Path,
    genome_ids: list[str],
    conn: duckdb.DuckDBPyConnection | None = None,
) -> dict[str, int]:
    """
    Export nucleotide sequences per genome.
    ...
    for genome_id in genome_ids:
        output_fna = output_dir / f"{genome_id}.fna"
        count = export_nt_fasta(output_fna, dataset_id=genome_id, conn=conn)
